            ...
            for pattern in patterns:
                try:
                    if re.search(pattern, entry):
                        should_skip = True
                        if DEBUG and (
                            ".gradle" in entry or "build" in entry or "gradle" in entry
                        ):
                            print(
                                f"  DEBUG: MATCHED pattern '{pattern}' - will skip '{entry}'"
                            )
                        break
                    elif DEBUG and (
                        ".gradle" in entry or "build" in entry or "gradle" in entry
                        ...
                        print(
                            f"  DEBUG: Did NOT match pattern '{pattern}' against '{entry}'"
                        )
                except re.error:
                    if DEBUG:
                ...
                for pattern in patterns:
                    try:
                        if re.search(pattern, rel_path):
                            ...
                            if DEBUG and (
                                ".gradle" in rel_path
                                or "build" in rel_path
                                or "gradle" in rel_path
                            ):
                                print(
                                    f"  DEBUG: MATCHED path pattern '{pattern}' - will skip '{rel_path}'"
                                )
                            break
                    except re.error:
                        continue
...
class HeaderManager:
    ...
    def create_header(cls, filepath, extra_content=None):
        ...
        if extra_content:
            header_lines.extend(
                f"{comment['start']}{line}{comment['end']}" for line in extra_content
            )
    ...
    def get_header_pattern(cls, file_ext):
        ...
        pattern = (
            rf"^{start}=== WATCHER HEADER START ==={end}\n"
            rf"(?:{start}[^\n]*{end}\n)*"  # Match any number of comment lines
            rf"{start}=== WATCHER HEADER END ==={end}\n?"
        )
    ...
    def should_process_file(cls, filepath):
        ...
        if filename in [
            cls.SCRIPT_NAME,
            cls.WATCHLIST_NAME,
            cls.DONOTWATCHLIST_NAME,
            cls.CURSORRULES_NAME,
        ]:
            return False
        ...
        for pattern in cls.get_donotwatch_patterns():
            try:
                ...
                print(
                    f"Warning: Invalid regex pattern in {cls.DONOTWATCHLIST_NAME}: {pattern}"
                )
                continue
        ...
        if not os.path.exists(cls.CURSORRULES_NAME):
            try:
                with open(cls.CURSORRULES_NAME, "w", encoding="utf-8") as f:
                    f.write(
                        "# This file will be automatically updated with the project tree structure\n"
                    )
                print(f"Created {cls.CURSORRULES_NAME}")
            except Exception as e:
                raise CursorRulesError(
                    f"Failed to create {cls.CURSORRULES_NAME}: {str(e)}"
                )
    ...
    def verify_watchlist(cls):
        ...
        if not os.path.exists(cls.DONOTWATCHLIST_NAME):
            ...
            with open(cls.DONOTWATCHLIST_NAME, "w") as f:
                f.write(
                    "# List regex patterns for files/paths to exclude (one per line)\n"
                )
                f.write("# Lines starting with # are ignored\n")
                ...
                f.write(
                    "# .*secret.*     # Excludes any file with 'secret' in the path\n"
                )
        ...
        if new_files:
            ...
            for dirpath in watched_dirs:
                if os.path.exists(dirpath):
                    self._observer.schedule(self, dirpath, recursive=True)  # type: ignore
...
def start_watching():
    try:
        ...
        print(
            f"Note: {HeaderManager.CURSORRULES_NAME} will be automatically updated with project tree"
        )
