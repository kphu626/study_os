...
class TaskModule(BaseModule):
    ...
    def _save_tasks_to_file(self):
        ...
        try:
            ...
            self.data_path.write_text(
                json.dumps(tasks_to_save, indent=2), encoding="utf-8"
            )
    ...
    def build_dpg_view(self, parent_container_tag: str):
        ...
        with dpg.group(parent=parent_container_tag):
            ...
            add_btn = dpg.add_button(
                label="Add Task", callback=self._dpg_add_task_callback, width=200
            )
    ...
    def _dpg_display_tasks_list(self):
        ...
        if not self.tasks:
            dpg.add_text(
                "No tasks yet! Add one above.", parent=self.dpg_tasks_list_container_tag
            )
        else:
            ...
            for task in sorted_tasks:
                with dpg.group(
                    horizontal=True, parent=self.dpg_tasks_list_container_tag
                ):
                    # Unique tag for the checkbox if direct manipulation is needed later, otherwise user_data is fine
                    ...
                    dpg.add_checkbox(
                        ...
                        },  # Pass task_id and its checkbox tag
                        callback=self._dpg_toggle_task_completion_callback,
                    )

                    task_text = task.description
                    if task.completed:
                        # Simple strikethrough (not directly supported, so we prepend)
                        # For actual strikethrough, would need custom text rendering or specific DPG features
                        # dpg.add_text(f"[DONE] {task_text}") # Basic indication
                        # Or, change color, etc.
                        text_item = dpg.add_text(task_text)
                        # dpg.configure_item(text_item, color=(150,150,150)) # Example: Dim completed tasks
                    else:
                        dpg.add_text(task_text)
        ...
        if dpg.does_item_exist(self.dpg_status_text_tag):
            dpg.set_value(
                self.dpg_status_text_tag, f"Displayed {len(self.tasks)} tasks."
            )
    ...
    def _dpg_add_task_callback(self, sender, app_data, user_data):
        ...
        if not description.strip():
            dpg.set_value(
                self.dpg_status_text_tag, "Error: Task description cannot be empty."
            )
            return
        ...
        try:
            ...
            new_task_data = TaskCreate(
                description=description, deadline=current_deadline
            )
